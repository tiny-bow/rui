...
const Context = rui.backend.Context;
const Size = rui.Size;
const Vertex = rui.Vertex;
...
const VTableTypes = struct {
    ...
    pub const drawClippedTriangles = *const fn (ctx: Context, texture: ?rui.Texture, vtx: []const Vertex, idx: []const u16, clipr: ?rui.Rect) void;

    pub const textureCreate = *const fn (ctx: Context, pixels: [*]u8, width: u32, height: u32, interpolation: rui.enums.TextureInterpolation) rui.Texture;
    pub const textureDestroy = *const fn (ctx: Context, texture: rui.Texture) void;
    pub const textureFromTarget = *const fn (ctx: Context, texture: rui.TextureTarget) rui.Texture;

    pub const textureCreateTarget = *const fn (ctx: Context, width: u32, height: u32, interpolation: rui.enums.TextureInterpolation) error{ OutOfMemory, TextureCreate }!rui.TextureTarget;
    pub const textureReadTarget = *const fn (ctx: Context, texture: rui.TextureTarget, pixels_out: [*]u8) error{TextureRead}!void;
    pub const renderTarget = *const fn (ctx: Context, texture: ?rui.TextureTarget) void;
...
/// Called by rui during `rui.Window.begin`, so prior to any rui
/// rendering.  Use to setup anything needed for this frame.  The arena
/// arg is cleared before `rui.Window.begin` is called next, useful for any
...
/// Called during `rui.Window.end` before freeing any memory for the current frame.
...
/// settings).  Currently only called during `rui.Window.init`, so currently
...
/// normalized (0-1).
pub fn drawClippedTriangles(self: *Backend, texture: ?rui.Texture, vtx: []const Vertex, idx: []const u16, clipr: ?rui.Rect) void {
    return self.vtable.drawClippedTriangles(self.ctx, texture, vtx, idx, clipr);
}

/// Create a `rui.Texture` from the given `pixels` in RGBA.  The returned
/// pointer is what will later be passed to `drawClippedTriangles`.
pub fn textureCreate(self: *Backend, pixels: [*]u8, width: u32, height: u32, interpolation: rui.enums.TextureInterpolation) rui.Texture {
...
/// pointer will not be used by rui.
pub fn textureDestroy(self: *Backend, texture: rui.Texture) void {
    return self.vtable.textureDestroy(self.ctx, texture);
}

/// Create a `rui.Texture` that can be rendered to with `renderTarget`.  The
/// returned pointer is what will later be passed to `drawClippedTriangles`.
pub fn textureCreateTarget(self: *Backend, width: u32, height: u32, interpolation: rui.enums.TextureInterpolation) !rui.TextureTarget {
...
pub fn textureReadTarget(self: *Backend, texture: rui.TextureTarget, pixels_out: [*]u8) !void {
...
/// used by rui.
pub fn textureFromTarget(self: *Backend, texture: rui.TextureTarget) rui.Texture {
    return self.vtable.textureFromTarget(self.ctx, texture);
}

/// Render future `drawClippedTriangles` to the passed `texture` (or screen
/// if null).
pub fn renderTarget(self: *Backend, texture: ?rui.TextureTarget) void {
...
/// Called by `rui.refresh` when it is called from a background
/// thread.  Used to wake up the gui thread.  It only has effect if you
/// are using `rui.Window.waitTime` or some other method of waiting until
