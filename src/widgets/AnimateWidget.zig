...
pub const InitOptions = struct {
    ...
    duration: i32,
    easing: ?*const rui.easing.EasingFn = null,
};
...
pub fn install(self: *AnimateWidget) !void {
    ...
    if (rui.animationGet(self.wd.id, "_end")) |a| {
        self.val = a.value();
    } else if (rui.animationGet(self.wd.id, "_start")) |a| {
        self.val = a.value();
    }
    ...
    if (self.val) |v| {
        switch (self.init_opts.kind) {
            .alpha => {
                self.prev_alpha = rui.themeGet().alpha;
                // alpha crashed if v is not between 0 and 1, which some easing functions may output
                rui.themeGet().alpha *= std.math.clamp(v, 0, 1);
            },
            .vertical => {
                if (rui.minSizeGet(self.wd.id)) |ms| {
...
pub fn start(self: *AnimateWidget) void {
    rui.animation(self.wd.id, "_start", .{
        .start_val = 0.0,
        .end_val = 1.0,
        .end_time = self.init_opts.duration,
        .easing = self.init_opts.easing orelse rui.easing.linear,
    });
}

pub fn startEnd(self: *AnimateWidget) void {
    rui.animation(self.wd.id, "_end", .{
        .start_val = 1.0,
        .end_val = 0.0,
        .end_time = self.init_opts.duration,
        .easing = self.init_opts.easing orelse rui.easing.linear,
...
pub fn deinit(self: *AnimateWidget) void {
    if (self.val) |v| {
        switch (self.init_opts.kind) {
            .alpha => {
                rui.themeGet().alpha = self.prev_alpha;
            },
            .vertical => {
